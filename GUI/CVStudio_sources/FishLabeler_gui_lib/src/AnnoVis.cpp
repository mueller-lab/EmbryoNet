...
void AnnoVis::redraw(const size_t& imageIndx)
{
	...
	{
		positions.append({ float(embryoBox.bbox.center().x()) / float(img.width()),
						   float(embryoBox.bbox.center().y()) / float(img.height()) });
		rotations.append(embryoBox.rotation);
	}

	emit sendArrows(positions, rotations);
...
void AnnoVis::swapRotation(const QPoint& pos)
{
	...
	{
		idToRelabel = m_imagesEmbryoBoxes[m_currentIndex][closestIdx].id;
		float value = m_imagesEmbryoBoxes[m_currentIndex][closestIdx].rotation.x() + 180;
		m_imagesEmbryoBoxes[m_currentIndex][closestIdx].rotation.setX(float(int(value) % 360));
		m_imagesEmbryoBoxes[m_currentIndex][closestIdx].isKeyFrame = true;
		emit(sendRotation(m_imagesEmbryoBoxes[m_currentIndex][closestIdx].rotation));
		updateRotation(idToRelabel, m_imagesEmbryoBoxes[m_currentIndex][closestIdx].rotation);
		//TODO fix between axes 
	}
...
void AnnoVis::setRotation(const QVector3D& rotation)
{
	...
	{
		idToRelabel = idsToSetRotation[0];
	}

	updateRotation(idToRelabel, rotation);
