...
def save_fm(x1, base_fm_name):
    path_to_fms = "F:\\Test\\Nodal\\"
    path_to_save_folder = os.path.join(path_to_fms, base_fm_name)
    os.makedirs(path_to_save_folder, exist_ok=True)
    for i in range(x1.shape[1]):
        img = (x1[0, i, :, :]).cpu().detach().numpy()
        img = (img - np.min(img)) / (np.max(img) - np.min(img))
        name_to_save = os.path.join(path_to_save_folder, str(i) + ".png")
        cv2.imwrite(name_to_save, np.uint8(img * 255))
...
class ResidualModule(nn.Module):
    ...
    def forward(self, x, name):
        ...
        x1 = self.conv_1_1(x)
        save_fm(x1, name + "_conv_11")

        x1 = self.bn_1_1(x1)
        x1 = F.relu(x1)
        save_fm(x1, name + "_relu_11")
        x1 = self.conv_1_2(x1)
        save_fm(x1, name + "_conv_12")

        x1 = self.bn_1_2 (x1)
        x1 = F.relu(x1)
        save_fm(x1, name + "_relu_12")

        x1 = self.conv_1_3(x1)
        save_fm(x1, name + "_conv_13")
...
class EmbryoNet2(nn.Module):
    ...
    def forward(self, x, t):
        ...
        x = self.base_conv(x)
        save_fm(x, "base_conv" + "7x7")
        x = self.base_bn(x)

        x = F.relu(x)
        x = F.max_pool2d(x, kernel_size=3, padding=1, stride=2)
        save_fm(x, "max_pool2d")
        x = self.M1(x,"resnet_block_1_1")
        save_fm(x, "resnet_block_1_1")
        x = self.M2(x,"resnet_block_1_2")
        save_fm(x, "resnet_block_1_2")
        x = self.M3(x,"resnet_block_2_1")
        save_fm(x, "resnet_block_2_1")
        x = self.M4(x,"resnet_block_2_2")
        save_fm(x, "resnet_block_2_2")

        x = self.M5(x,"resnet_block_3_1")
        save_fm(x, "resnet_block_3_1")
        x = self.M6(x,"resnet_block_3_2")
        save_fm(x, "resnet_block_3_2")
        x = self.M7(x,"resnet_block_4_1")
        save_fm(x, "resnet_block_4_1")
        x = self.M8(x,"resnet_block_4_2")
        save_fm(x, "resnet_block_4_2")
