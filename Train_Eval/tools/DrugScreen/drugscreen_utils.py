...
class DrugScreenExcelReader:
    ...
    def file_excel_load(self):
        """Load excel file."""
        return pd.read_excel(self.path_src, names=self.parameters['cols_sheet'])
    ...
    def init_params(self):
        ...
        if self.num_screen == 2840:
            parameters = {'colors': parameters['colors'],
                          'cols_sheet': parameters['cols_temp_2840'] + parameters['cols_detections']}
        elif self.num_screen == 2843:
            parameters = {'colors': parameters['colors'],
                          'cols_sheet': parameters['cols_temp_2843'] + parameters['cols_detections']}
        else:
...
class DrugScreenPhenotypeAnalyzer(DrugScreenExcelReader):
    ...
    def __init__(self, num_model, num_plate, num_screen, path_src, path_dst):
        ...
        self.screen_title = f'SCREEN-WELL® ICCB Known Bioactives library'
    ...
    def plot_charts(self, df, mode, donut_plot=False, filename=None, plot_legend=True, title=None, save_fig=False):
        ...
        donut_plot: bool
            Indicates whether pie chart should be plotted as donut plot or not.
        filename: str
            Optional filepath to save figure to. If no filename is specified, default filename will be created.
        mode: int
            Indicate whether a pie chart or bar chart should be generated.
            Mode 0 indicates pie chart, mode 1 indicates bar chart.
        plot_legend: bool
            Indicates whether legend should be displayed or not. Default is set to True.
        title: str
            Optional title to be displayed with figure.
        save_fig: bool
            Indicate whether figure should be saved to file or not. Default is set to False.
        ...
        if mode == 0:
            ax1, patches = self.plot_pie_chart(fig,
                                               counts_sorted,
                                               colors_sorted,
                                               explode_sorted,
                                               labels_sorted,
                                               ...
                                               fontsize=12)
            if plot_legend:
                plt.legend(patches, labels_sorted, loc='best', fontsize=8)
        elif mode == 1:
            _ = self.plot_bar_chart(fig, counts_sorted, colors_sorted, labels_sorted)
        else:
            print('Invalid mode.')
        ...
        if save_fig:
            if filename is None:
                filename = f'{str(self.path_output)}_overview{self.fileformat}'
            else:
                pass
    ...
    @staticmethod
    def plot_bar_chart(fig, counts, colors, labels):
        """Plot a bar chart based on count values."""
        ax = fig.add_subplot(111)
        num_bars = np.arange(len(counts))
        ax.bar(num_bars, counts, color=colors, tick_label=labels, width=0.5)
        heights = 0
        for p in ax.patches:
    ...
    def plot_pie_chart(self, fig, counts, colors, explode, labels, position, donut_plot=False, fontsize=10):
        """Plot a pie chart based on count values."""
        ax = fig.add_subplot(position[0], position[1], position[2])
        patches, texts, _ = ax.pie(counts,
    ...
    def plot_plate_exact(self, df, filename=None, title=None, save_fig=False):
        """
        ...
        for index, row in df.iterrows():
            ...
            drugname = self.fn_drugname(row['Name'])
    ...
    def plot_plate_majority_vote(self, df, filename=None, title=None, save_fig=False):
